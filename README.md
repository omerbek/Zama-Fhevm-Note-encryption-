...
Welcome to the Blockchain Note Simulator – a demo app that shows how notes can be securely “stored” on a blockchain using hashed values.
Important: This is a simulation. It does not use real Full Homomorphic Encryption (FHE).

**✏️ Write a Note**

Type your note in the text box (example: "My secret password is 123456")
Click "Encrypt & Save to Blockchain"
Your note is converted into a hash and sent to the Sepolia testnet
The smart contract’s setNote() function stores it permanently on-chain
...
Why?
Only the hash is stored, the actual content is never saved
Hashes are one-way functions (SHA-3), impossible to reverse
...
# Zama template'ini klonlayın

```bash
git clone https://github.com/zama-ai/fhevm-hardhat-template
```

# Dependencies ve FHEVM kütüphanelerini yükleyin
```bash
npm install
...
contracts/EncryptedNotebook.sol dosyasını oluşturun:
...
#  Hardhat Config Dosyasını Düzenleyin
hardhat.config.js dosyasını açın ve aşağıdaki gibi düzenleyin:
...
module.exports = {
  ...
  networks: {
    // Zama Testnet için konfigürasyon
    zamatest: {
      url: "https://devnet.zama.ai", // Zama devnet URL
...
# Deploy Script'i Oluşturun
scripts/deploy.js dosyasını düzenleyin:
...
async function main() {
  console.log("🚀 Zama EncryptedNotebook dApp Deploy...");
  ...
  console.log("✅ EncryptedNotebook deployed to:", address);
  console.log("📝 Transaction hash:", notebook.deploymentTransaction().hash);
  
  // FHEVM instance oluştur
...
```bash
# Sepolia RPC URL (Infura veya Alchemy'den alın)
SEPOLIA_RPC_URL=https://eth-sepolia.g.alchemy.com


# Private key (0x... şeklinde)
PRIVATE_KEY=0x2fc6888

# Sepolia Etherscan API key (verify için)
...
# src/App.css dosyasını düzenleyin:
...
<img width="1245" height="989" alt="image" src="https://github.com/user-attachments/assets/0f822fe7-6c55-49c9-ad82-aa268652b6b7" />
